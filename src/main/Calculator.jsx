...
export default class Calculator extends Component {
    ...
    setOperation(operation) {
        if (this.state.current === 0) {
            ...
            try {
                // Avaliando o resultado do cálculo
                let result = eval(`${values[0]} ${currentOperation} ${values[1]}`);
    
                // Se o resultado é um número de ponto flutuante, arredonda apenas se necessário
                if (!Number.isInteger(result)) {
                    result = parseFloat(result.toFixed(8)); // Arredonda para até 8 casas decimais apenas se não for um inteiro
                }
